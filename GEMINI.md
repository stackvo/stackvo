...
> [!IMPORTANT] > **Kod içi yorumlar (inline comments) her zaman İngilizce olmalıdır**

- Bash script'lerdeki `#` ile başlayan yorumlar İngilizce olmalıdır
- PHP, JavaScript, CSS ve diğer tüm programlama dillerindeki yorumlar İngilizce olmalıdır
- Sadece fonksiyon başlıklarındaki dokümantasyon blokları Türkçe olabilir (örn: `##` ile başlayan bloklar)
- Inline yorumlar (kod satırlarının yanındaki veya arasındaki yorumlar) mutlaka İngilizce olmalıdır
...
generate_project() {
    ...
    # Proje var mı kontrol et  ❌ YANLIŞ - İngilizce olmalı
    if [ -d "$project_name" ]; then
        log_error "Proje zaten mevcut"
        return 1
    fi

    # Proje dizini oluştur  ❌ YANLIŞ - İngilizce olmalı
    mkdir -p "$project_name"
}
```
...
4. **Inline Yorumlar**: Her zaman İngilizce kullan
   ```bash
...
2. **Implementation**: Kodu yaz, İngilizce inline yorumlar ekle
