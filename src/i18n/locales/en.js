export default {
  ...
  projectSupervisor: {
    ...
    logToStdout:
      'This process writes to the container\u2019s stdout, which supervisord cannot read back. Below is the container\u2019s own output, every process mixed.',
    pending:
    ...
    daemon: {
      ...
      status: 'Status',
      reread: 'Re-read the config',
      update: 'Apply config changes',
      'start-all': 'Start all',
      'stop-all': 'Stop all',
      'restart-all': 'Restart all',
      silent: '(supervisorctl said nothing)',
      confirm: {
        'stop-all': {
          title: 'Stop every process?',
          message:
            'This stops php-fpm and the web server too: {name} goes down until something is started again.',
        },
        'restart-all': {
          title: 'Restart every process?',
          message:
            'php-fpm and the web server restart with the rest: {name} drops its connections for a moment.',
        },
      },
    },
    detail: {
      ...
      runtime: 'Now',
      declared: 'Declared in stackvo.json',
      config: 'In the container\u2019s config',
      resources: 'Cost',
      ...
      pid: 'pid',
      uptime: 'Uptime',
      description: 'supervisord says',
      restarts: 'Restarts seen',
      exec: 'Command',
      replicas: 'Copies',
      stopWait: 'Stop timeout',
      enabled: 'Enabled',
      yes: 'yes',
      no: 'no',
      memory: 'Memory (RSS)',
      cpu: 'CPU, lifetime average',
      cpuSeconds: 'CPU time',
      threads: 'Threads',
      noConfig: 'The config file in the container has no block for this process.',
      noResources: 'Nothing to measure \u2014 the process has no pid.',
      unknownHint:
        'Neither the image nor stackvo.json declares this process. It was added inside the container, and the next rebuild drops it \u2014 declare it under `processes` in the manifest to keep it.',
      origin: {
        image: 'from the image',
        manifest: 'from stackvo.json',
        unknown: 'not declared',
      },
      signals: {
        HUP: 'Hang up \u2014 most programs re-read their config or reopen their logs',
        INT: 'Interrupt \u2014 the same as Ctrl+C in a terminal',
        QUIT: 'Quit \u2014 exit and leave a core dump; nginx finishes open requests first',
        USR1: 'User signal 1 \u2014 program-defined; php-fpm and nginx reopen their log files',
        USR2: 'User signal 2 \u2014 program-defined; php-fpm reloads its workers gracefully',
        TERM: 'Terminate \u2014 asks the process to exit cleanly; what a stop sends',
        KILL: 'Kill \u2014 ended by the kernel at once, no cleanup, nothing saved',
      },
    },
  },
  supervisorCheck: {
