...
<template>
  <!-- A stack of cards, not one card holding others: the four measurements, the
     composition and the activity grid are three views of the same container
     and none of them is inside another. -->
  <v-alert type="success" variant="tonal" class="mb-4" :icon="running ? 'mdi-pulse' : 'mdi-pause'">
    {{ running ? t('projectDetail.live') : t('projects.stopped') }}
  </v-alert>

  <v-row>
    <v-col cols="12" sm="6" lg="3">
      <v-card variant="flat" class="pane metric-tile">
  ...
  <v-card variant="flat" class="pane mt-4">
    <PaneHeader
      help="project-indicator-composition"
      icon="mdi-chart-donut"
      :title="t('projectDetail.composition')"
      :description="t('projectDetail.compositionExplain')"
    />
  ...
  <v-card variant="flat" class="pane">
    <PaneHeader
      help="project-indicator-cpu-activity"
      icon="mdi-calendar"
      :title="t('projectDetail.cpuActivity')"
      :description="t('projectDetail.cpuActivityExplain')"
    />
