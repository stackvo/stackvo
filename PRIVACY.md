...
after the first fetch, so the panel works offline for every document it has
opened before. Nothing ships inside the installer: a document opened for the
first time with no network is reported as unreachable, not shown from an older
copy.
...
| Open a help panel               | `raw.githubusercontent.com`, for the help document of the card you opened it on. The request carries the topic name (`project-tunnel`, `settings-dns`) and your interface language, and no identifier of any kind. **It says which help page you opened**, which is a fact about what you were stuck on — so it is here rather than buried. One request per topic per run: the answer is cached under the app's own directory and the panel works offline from that cache. Nothing ships inside the installer, so a document opened for the first time with no network is reported as unreachable rather than shown from an older copy. A failed request for a document already cached is never reported; it silently reads what is on disk                                                                                                                                                                                                        |
