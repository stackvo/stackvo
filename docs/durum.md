...
| R-3 | **Konteynerin bir editör taşıyabilmesi** | 🟡 | **Ön koşulun kendisi indi: `editor.rs`.** Satırın saydığı dört sorudan ikisi kod gerektiriyordu ve ikisi de yazıldı. **Kaynak** — sessizce yanlış olan tek hâl — artık bir uyarı değil bir **ret**: `readiness()` konteynerin kendi mount tablosunu okuyor ve çalışma dizini host'tan bind değilse editörü açtırmıyor. Varsayımla değil gözlemle: manifest'in söylediği ile çalışan konteynerin söylediği ayrılabiliyor (dev server'ı açmak örtü yazıyor, örtü konteyner yeniden kurulana kadar hiçbir şey yapmıyor) ve ayrıldığında doğru olan konteynerdir. Yanlış yoldaki bir bind (`/var/log` her PHP konteynerinde var) ya da çalışma dizinindeki bir **named volume** (`perf.rs` tam olarak host kopyasının artık okunmadığı yer) kaynak sayılmıyor; duran bir konteyner ise yalnız "durmuş" diyor — boş bir mount listesinden ret üretmek, hiçbir şey sormamış olmaktır. **Kalıcılık** indi: `docker-compose.editor.yml`, proje başına bir adlandırılmış volume, `/root/.vscode-server` üstünde. Örtü, çünkü generator çıktısı Bash sürümüyle bayt bayt sözleşmeli. Ve kimse editör açmasa da mount içeride — `debugbridge`'in üç mount'u için yaptığı aynı gerekçe: volume, konteyneri yeniden kurmayı gerektiren yarıdır, ilk basışta eklemek düğmenin uygulamayı yeniden başlatması demektir. **libc** iddiadan çıkıp imajdan okunuyor (`node:X-alpine` tek kaynağa indi, `generator::node_base_image`), ve alpine bir ret değil bir kayıt: VS Code'un musl derlemesi var, JetBrains'inki yok — o R-2'nin. **Lang runtime'ları dışarıda ve gerekçesiyle:** `COPY . .` ile kuruluyorlar ve `devserver`'ın örtüsünün karşılığı yok, yani kaynağı hiç taşıyamıyorlar; reddedilecek bir editör için yüz megabaytlık volume boşuna. **Kapılar:** `editor_claims.rs` (4 test) generator'daki iki sessiz varsayımı tutuyor — hiçbir üretilen Dockerfile `USER` satırı taşımıyor (`$HOME` = `/root`, yoksa volume kimsenin yazmadığı bir dizini saklar ve indirme sonsuza dek tekrarlanır), ve PHP kaynağı mount ediyor, snapshot runtime'ları etmiyor; ikisinin de ısırdığı kanıtlandı. `editor.rs`'in kendi yargısı 13 birim testiyle ölçülü. **Kalan: R-1'in adresi ve düğmesi** |
...
**Kod yazılacak olanlar** — bir tane, ve küçük değil. İkisiydi; **B-1** kapandı
ve bu tablodan çıktı — kaydı `CHANGELOG.md`'de. Kalan pencerenin `exec_ts`
varsayımı koddan kaldırıldı: StackVo'nun kendi başlattığı bir kayıt, isteğin iki
yanındaki host saatini taşıyor, ve tarayıcıda alınmış bir kayıt için kalan
hesaplanmış yedek ekranda **hesaplandı** diye adlandırılıyor
(`examples/explain_probe.rs` o yedeğin varsayımını canlı bir konteynere karşı
sormaya devam ediyor).

* **§2 R-1 + R-3** — editörün konteynerin içinde koşması, VS Code tarafı.
  **R-3'ün yarısı indi:** `editor.rs` kaynağın gerçekten mount olup olmadığını
  konteynerin kendi mount tablosundan okuyor ve olmadığında reddediyor, libc'yi
  imajdan çıkarıyor, ve sunucu için adlandırılmış volume'u bir örtüyle
  koyuyor. Kalan R-1'in adresi. Tersi, bir düğmenin editörü Node imajının içindeki *ölü
...
* **§2 W / #35** — testlerin Windows'ta **yeşile dönmesi**. Log okundu: on
  dokuzdan geriye **bir** hata kalmıştı ve o da düzeltildi (`agents::home()`,
  `%USERPROFILE%`). Kalan tek şey bir koşunun bunu doğrulaması.
...
| | Sayı | Nasıl sayıldı |
|---|---|---|
| Toplam IPC komutu | **306** | `contracts/ipc.json` → `commands` (302 Rust + 1 ertelenmiş + 3 `frontend-plugin`) |
| Bunlardan `#[tauri::command]` olarak yazılmış | **302** | `commands.rs`, `#[cfg(test)]` dışı |
| Frontend kaynak dosyası | **154** | `src/**/*.{js,vue}`, spec dosyaları hariç |
...
| Rust kaynağı | **111 modül, 110.465 satır** | `src-tauri/src/*.rs` |
