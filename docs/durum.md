...
| B-7 | **Tünelde kimlik ve kalıcı ad** | ✅ | **Bitti.** Kimlik doğrulama sağlayıcıdan istenmedi — dokuzun dördü kendisi yapabiliyor, üçü hiç yapamıyor, yapabilen dördü dört ayrı yazımla — bunun yerine kimlik bilgisi varken yan konteyner **StackVo'nun kendi nginx bekçisine** (`tunnelid.rs`) yönlendiriliyor; Tailscale dahil, çünkü onun ağ ad alanı da artık projenin değil bekçinin. Ölçüldü, `examples/tunnel_guard_probe.rs` gerçek konteynerlerle: kimlik bilgisiz → 401 ve tarayıcının soracağı `WWW-Authenticate`, yanlış kimlik → 401, doğru kimlik → uygulamanın kendi baytları (896 = 896), ve kimlik bilgisi ulaşmayan bekçi **açık gelmek yerine başlamayı reddediyor**. Parola üretiliyor (telefonda yanlış okunan karakterler olmadan yirmi karakter), anahtar deposunda proje başına duruyor ve konteynere ortam değişkeniyle gidiyor — çalışma alanında dosya yok, argümanda değer yok. **Kalıcı ad** her istemcinin gerçekten sahip olduğu bayrakla gönderiliyor (`--subdomain`, `--url=`, `--hostname=`, zrok'ta ayır-sonra-paylaş) ve **verilip verilmediği kontrol ediliyor**: localtunnel aynı alt alan adını doksan saniye arayla iki kez geri verdi, ama bir önceki tünel yeni kapanmışken **sessizce** `bitter-bulldog-88.loca.lt` verdi — çalışan bir tünel ve hiçbir yere bakmayan bir panel kaydı. Dokuzuncu sağlayıcı bir satır olarak geldi: `cloudflare_named`, hızlı tünelin ikiz istemcisi, `TUNNEL_TOKEN`'ı ortamdan kendisi okuyor (ölçüldü) ve reddi (`Provided Tunnel token is not valid.`) `find_failure`'ın bilmediği bir cümleydi. Ölçüm iki hatayı da kod tarafında yakaladı: tırnaksız heredoc komut ikamesi yapıyor, yani bir nginx **yorumundaki** ters tırnak bekçiyi başlatmıyordu; ve kimlik bilgisiyle anahtarlanan bir nginx `map`'i, gerçekten seçilebilecek uzunlukta bir parolada `could not build map_hash` ile reddediliyordu. **Ölçülmeyen tek şey**, hesap isteyen beş sağlayıcıda *geçerli* bir jetonun kalıcı adı gerçekten verip vermediği — ki bu zaten `verified` alanının söylediği şey |
...
| --- | --- | :-: | --- |
| 2 | Güncelleme endpoint'i | 🟡 | **Tören yapıldı, sürüm yayınlandı, dosyalar yok.** ADR 0025 endpoint'i düzeltti, ADR 0033 töreni yazdı (`tools/keys.sh`), iki anahtar da üretilip yerine kondu — updater'ınki `tauri.conf.json`'da, içerik anahtarı `signing::PINNED`'de, ve `key_ceremony.rs` ikisinin aynı olduğu bir derlemeyi reddediyor. Secret'lar eklendi: `v0.1.0` etiketi **Release #1'i tetikledi ve `preflight` geçti**, ki bu `TAURI_SIGNING_PRIVATE_KEY`'in yerinde olduğunun kanıtı. Ama altı hedefin altısı da `Verify the contract surface` adımında düştü (Windows testleri, W), yani release'te yalnız GitHub'ın kendi kaynak arşivleri var ve `releases/latest/download/latest.json` hâlâ **404**. Kalan kod değil: W yeşile dönünce yeni bir etiket |
| 22 | Platform kapsamı (Linux aarch64, Win ARM64) | 🟡 | **Prova gerçek bir yayınla koştu ve iki ARM satırının ilk sorusu cevaplandı.** `v0.1.0` Release #1'i tetikledi; `ubuntu-24.04-arm` ve `windows-11-arm` işleri **başladı, derledi ve test koştu** (11 ve 18 dakika), yani koşucu etiketleri çözülüyor ve toolchain'ler orada çalışıyor — koşucusuz doğrulanamayan tam da buydu. Cevaplanmayan yarısı: **bundler orada mutlu mu**, çünkü altı iş de paketleme adımına varmadan test adımında düştü. Kalan: W yeşile dönünce bir etiket daha |
...
* **§2 W / #35** — testlerin Windows'ta **yeşile dönmesi**. Koşmaya başladılar
  ve on dokuz hata gösterdiler; ikisi üründü ve düzeltildi, kalanı ayraç ve
  kabuk varsayımıydı. CI #38'de yedi işten altısı yeşil, `windows-latest`
  `Test` adımında duruyor. Kalan kaç hata olduğu bu depodan okunmuyor —
  log'a giriş gerekiyor.
* **§2 Y-1, Y-2** — ekran okuyucu ve yerel pencere denetimi. Y-2 W ile aynı
  makineye bağlı: `tauri-driver` macOS'ta koşmuyor.
...
* **#2** — tören yapıldı, anahtarlar yerinde, secret'lar eklendi ve `v0.1.0`
  yayınlandı; `preflight` geçtiği için özel anahtarın orada olduğu **ölçüldü**.
  Ama altı hedefin altısı da test adımında düştüğü için release'te dosya yok ve
  endpoint 404. Kalan tek eylem: **W yeşile dönünce bir etiket daha.** O tek
  adım #22'nin kalan yarısını, #21'i ve §2 C'nin önünü birlikte açıyor.
