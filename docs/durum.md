...
On dört rakibe karşı ölçüldü (DDEV, Lando, Yerd, KettleCode, DevTent, dde.sh,
Laradock, Herd, Lerd, ServBay, EnvKit, Laragon, FlyEnv, ForgeKit). Her satır
**nasıl bakıldığını** taşıyor: "yok" iddiası bir `grep` sonucu ya da bir dosya
listesi, "rakipte var" iddiası resmî dokümandan.

Yapılanlar burada durmuyor — `CHANGELOG.md` her teslimatın ne olduğunu ve neden
öyle yapıldığını taşıyor, ve bu tablo yalnız **kalanı** listeliyor.
...
Kod tarafı bitmiş, **söylenmemiş**. Hiçbiri mühendislik değil; her biri bir
sayfa ya da bir başlık.

* **MCP yüzeyi** — 34 araç. KettleCode 18 aracı, ServBay ve EnvKit birer
  sunucuyu manşet yapıyor; burada daha fazlası README'nin ortasında anlatılıyor.
  Ayrıca **AI kural dosyası yazan tek ürün**, ve rakiplerin hiçbirinde yok.
* **Üretim imajı dışa aktarma** (`release.rs`) — `laradock ship` dışında hiçbir
  rakipte yok, ve yerel-ikili rakiplerin hiçbiri takip edemez: konteyner
  soyağacı olmadan mümkün değil. README'nin yol haritasında bile geçmiyor.
* **Yan yana sürümler** (`instances.rs`) — Herd bunu "Service Version Matrix"
  diye bir doküman sayfasıyla satıyor, ServBay ve FlyEnv "anında sürüm
...
On dört rakibin **hiçbirinde** olmayanlar. Kalan iş değil — kaybedilmemesi
gereken şey, ve yazılı olmasının sebebi bu:

* **OAuth geri çağrıları** (`oauth.rs`) — yalnız Herd'de "Social
  Authentication" olarak var, o da yerel HTTPS'in bir yan etkisi olarak.
* **Stripe webhook'ları** (`stripe.rs`), **git worktree desteği**
  (`worktree.rs`), **dört motorda ajanısız sorgu günlüğü** (`querylog.rs`) —
  hiçbirinde yok.
* **MDM politika dosyası** (`policy.rs`) — hiçbirinde yok. Kurumsal alım kararı
  bu satırdan geçiyor.
* **İmzalı paket dağıtımı + hava boşluklu kurulum** — hiçbirinde yok.
* **EN 301 549 uyum beyanı ve onu tutan testler** — hiçbirinde yok. Kamu
  ihalesi bu satırdan geçiyor.
* **Sözleşmeyle kapılı üç yüzey** (CLI/TUI/MCP, hepsi `contracts/ipc.json`'a
  bağlı ve testlerle tutuluyor) — hiçbirinde yok.
* **Devralma planı** (`handover.rs`) — mevcut bir kurulumun verisini
  kaybetmeden yeni düzene taşımak. Rakiplerin "import"ları yeni kurulum
  yapıyor.
* **Üretim imajı dışa aktarma**, **yan yana sürümler**, **34 araçlık MCP
  yüzeyi**, **AI kural dosyası yazıcısı** — yukarıdaki "anlatılmayan güçler"le
  aynı liste, ve aynı sebeple burada: kod var, cümle yok.
* **MIT.** Herd $99/yıl, ServBay $59/yıl, Laragon ticarileşti, DevTent kendi
  lisansını yazdı. DDEV, EnvKit ve ForgeKit tam bu çizgiden saldırıyor.

### Girilmeyecek kavgalar

Yeniden tartışılmaması için yazılı. On dört ürünlük tur bu listeye ekleyecek
bir şey bulmadı ve **bir şeyi çıkardı**: "yerel AI servisleri" kapsam dışı
duruyordu, ama reddedilen şey ServBay'in **LLM ağ geçidi**ydi ve o hâlâ kapsam
dışı — Ollama ve Qdrant'ı birer *paket* olarak eklemek ayrı bir soruydu ve ADR
0027 onu kendi gerekçesiyle cevapladı.
...
* **Konteyner araçlarının host'a indirilmesi** (E'nin reddedilen yarısı):
  `composer`, `node`, `npm`, `bun`, `wp`. Sürüm projenin özelliği, host
  dizininin değil. `tooling::TOOLS` bunu bir yorumla değil bir testle tutuyor
  (`the_catalogue_is_host_tools_only`), çünkü buraya bir satır eklemek tam
  olarak birinin altı ay sonra iyi niyetle yapacağı şey.
...
Kapanmış maddeler bu tablodan **çıkarıldı** — ne yapıldığı ve neden öyle
yapıldığı `CHANGELOG.md`'de, geri alınamayan tercihler §6'da duruyor. Burası
yalnız kalanı taşıyor.
...
| 22 | Platform kapsamı (Linux aarch64, Win ARM64) | 🟡 | **Prova koştu, ve Linux ARM satırı neredeyse tamamen cevaplandı.** Altı hedefin üçü yeşil (iki macOS, `ubuntu-latest`), üçü kırmızı — ama kırmızıların hiçbiri “ARM'de derlenmiyor” demiyor. **`ubuntu-24.04-arm`: süit yeşil geçti**, uygulama 4 dk 54 sn'de derlendi, ve bundler **`StackVo_0.1.0_arm64.deb` ile `StackVo-0.1.0-1.aarch64.rpm`'i yazdı** — üç turdur beklenen iki paket, doğru mimari adlarıyla, çapraz derleme değil yerel koşucuda. AppImage adımı da araçlarını indirdi (`AppRun-aarch64`, `linuxdeploy-aarch64.AppImage`, gtk ve appimage eklentileri, hepsi çözüldü) ve **tek bir dosyada** düştü: `xdg-open binary not found /usr/bin/xdg-open`. **Ve bu kusur #22'nin tam şekli.** Bundler, uygulama bir bağlantı açabiliyorsa (`tauri-plugin-opener` açabiliyor) `/usr/bin/xdg-open`'ı **derleme makinesinden** AppDir'e kopyalıyor. `ubuntu-latest` `xdg-utils` taşıyor, `ubuntu-24.04-arm` taşımıyor. Yani soru hiçbir zaman “ARM derliyor mu” değildi; **“iki koşucu imajı aynı makine mi”** idi, ve cevap kimsenin öngöremeyeceği bir yerde hayırdı. `xdg-utils` apt satırına eklendi ve `release_rehearsal.rs` orada kalmasını tutuyor — çünkü bir `-dev` başlığı değil, listeye yanlışlıkla girmiş gibi duruyor. **Mekanizma tam tasarlandığı gibi çalıştı:** iki Windows satırında süit kırmızıydı ve **iş yine de paketlemeye gitti** — `continue-on-error`, sonra `always()` ile bundle'lar run sayfasına indi, sonra son adım süitin `outcome`'unu okuyup işi kırmızı bitirdi. Eski hâlinde bu üç iş test adımında durur, hiçbir şey bırakmaz ve bu tur da hiçbir şey öğretmezdi. **`x86_64-apple-darwin` yeşil**: altı satırın tek gerçek çapraz derlemesi paketlemeye kadar gitti. Bu, `rustup target add` düzeltmesinin *gerekli olduğunu* kanıtlamaz — önceki koşu oraya hiç varmamıştı, yani karşılaştırılacak bir hâl yok — ama o satırın hedefi artık derleyen toolchain'de duruyor ve satır yeşil. **Bir kusur da kontrolün kendisindeydi ve bu koşu buldu:** “What the bundler produced” adımı `steps.tauri.outcome == 'success'` ile kapılıydı, yani `ubuntu-24.04-arm`'da diskteki iki paket hakkında **hiçbir şey söylemeden atlandı** — yalnız hiçbir şey ters gitmediğinde konuşan bir kontrol, önemli olduğu anda hiç konuşmayan kontroldür. Kapı kaldırıldı. **Okunmamış olan:** iki Windows satırında `tauri-action`'ın **neden** düştüğü (9 dk 18 sn ve 7 dk 22 sn sonra). Log henüz açılmadı ve #2'nin dersi tam da bu — sebep okunmadan yazılmaz. Süit hatası ayrı bir sorun (§2 W) ve bu turda paketlemeyi artık durdurmuyor. **Ve asıl ders, teşhisten de önemlisi: bu kusurun GitHub'da bulunmasına gerek yoktu.** `tools/linux/run.sh`'in her modu ya derliyor ya test ediyordu; **hiçbiri paketlemiyordu**, `ci.yml` de paketlemiyor, ve konteyner imajı `ci.yml`'i aynalıyordu — yani imajın karşısında tutulmadığı tek liste, rehearse ettiği işi tarif eden tek listeydi. Üstelik Apple Silicon'da o konteynerin kendisi **`aarch64-unknown-linux-gnu`**: düşen satırın ta kendisi. Kusur hep buradan üretilebilirdi ve üretecek komut yoktu. Artık var: **`tools/linux/run.sh --bundle`** konteynerin kendi mimarisinin yükleyicilerini üretip `check-installers.mjs` ile yargılıyor; **`--windows-bundle`** NSIS yükleyicisini `cargo-xwin` ile çapraz üretiyor (MSI Windows dışında üretilemiyor ve bu `tauri-bundler`'ın kendi ayrımı — `msi` modülü `#[cfg(target_os = "windows")]`, `nsis` modülü ise Linux ve macOS'ta kurulabildiğini kendi yazıyor; zaten önemli olan yarı NSIS: updater `-setup.exe` indiriyor, `.msi` değil). İkisi de `tools/before-push.sh --all`'a bağlandı. Ve kuralı tutan şey: `workflow_parity.rs` artık `tools/linux/Dockerfile`'ı **`release.yml`'in** paket listesine karşı tutuyor, `ci.yml`'inkine değil — konteyner en az koşucu ortamı olmak zorunda. **Ve yeni komut işini ilk günü yaptı: Linux ARM yarısı bu makinede kapandı.** `tools/linux/run.sh --bundle` iki tur döndü ve iki duvarı arka arkaya yıktı — `xdg-utils` (koşucunun bulduğu), sonra `appimagetool`'un istediği `file` (koşucu imajının release'e bedava verdiği, hiçbir workflow'da yazmayan). İkincisinin dersi Dockerfile'a yazıldı: imaj **iki yönde birden** üst küme olmak zorunda — release'in bildirdiği her paket, artı koşucunun bedava verdiği her şey. Sonuç: **`aarch64-unknown-linux-gnu: deb, rpm, appimage — each present and named for aarch64`**, yani üç format da, doğru mimariyle, koşucu harcamadan. **İki Windows satırının sebebi de okundu, ve Windows'un ARM'le hiç ilgisi yokmuş.** `gh` ile log açıldı: ikisi de — x86_64 ve ARM64 — aynı satırda öldü, `Couldn't find a .ico icon`, hem de uygulama derlendikten **sonra**. `icons/icon.ico` ve `icons/icon.icns` depoda duruyordu; `bundle.icon` yalnız `icons/icon.png` diyordu, yani bundler o iki dosyadan hiç haberdar edilmemişti. Görünmemesinin sebebi kusurun **asimetrik** olması: macOS gerekirse PNG'den `.icns` **üretiyor**, Linux zaten PNG kullanıyor — üç platformun ikisi sessizce iyi, üçüncüsü sert hata, ve üçüncüsü kimsenin koşmadığı platform. Ekrandan bakınca `windows-11-arm` bir ARM sorunu gibi duruyordu; `windows-latest` de aynı yerde ölüyordu. Liste tauri'nin standart beşlisine çevrildi ve `release_rehearsal.rs` iki yönlü tutuyor: `.ico` ile `.icns` listede olmalı, **ve** listedeki her ad diskte bir dosya olmalı. **Altı satırın beşi artık bu makineden cevaplanıyor, ve altıncısının cevaplanamadığı ölçüldü.** Windows tarafında `--windows-bundle` NSIS'i `cargo-xwin` ile çapraz üretiyor — yol boyunca iki eksik ad kondu: Debian `clang-cl`'in kodunu paketliyor **adını** paketlemiyor (sürücü modunu `argv[0]`'dan seçtiği için symlink yetiyor), ve `llvm-ml` de x86_64'te `ring` ile `aws-lc-sys`'in MASM assembly'si için gerekiyor; ikisi olmadan `cc-rs` düz `clang`'e düşüyor ve her C dosyası bir bayrağı dosya sanan `no such file or directory: '/imsvc'` ile ölüyor. `cargo-xwin`'in indirdiği MSVC CRT'si de artık kalıcı bir hacimde, her koşuda yeniden inmiyor. **Ve `x86_64-pc-windows-msvc`'nin NSIS yükleyicisi de bu makinede üretildi:** `StackVo_0.1.0_x64-setup.exe`. Son engel yine bir ad meselesiydi — Windows'ta tauri kendi NSIS'ini indirip `makensis.exe`'yi çağırıyor, başka her yerde PATH'teki `makensis`'i, ve onu taşımıyor; verdiği hata yine `makensis.exe` diyor, yani bir `apt-get install` uzaklıktaki eksiği çapraz derleme çıkmazı gibi gösteriyor. Bu, aynı sınıftan **beşinci** bulgu: `xdg-utils`, `file`, `clang-cl`, `llvm-ml`, `nsis` — hepsi derleyicinin değil **bundler'ın** ihtiyacı, ve hiçbiri hiçbir workflow'da yazmıyor çünkü koşucu imajları onları bedavaya veriyor. **`aarch64-pc-windows-msvc` ise burada üretilemiyor ve bu bir ayar eksiği değil.** `ring`, ARM64 Windows assembly'sini `clang-cl` ile değil düz `clang` ile derliyor — MSVC assembler'ı o sözdizimini okumadığı için — ve düz `clang` Microsoft'un ARM64 başlıklarını okuyamıyor: `winnt.h`, `#define __MACHINEARM_ARM64 __MACHINE` satırında duruyor, `clang-cl` ise aynı dosyayı sorunsuz derliyor. İkisi aynı `cc::Build` içinde, ve ortak bir bayrak da yok — bu makinede ölçüldü: `clang` yalnız `-isystem`, `clang-cl` yalnız `-imsvc`/`/imsvc` kabul ediyor. `--windows-bundle aarch64-pc-windows-msvc` artık on dakika derleyip düşmek yerine bir saniyede bu gerekçeyi yazıp çıkıyor. O satır koşucunun ya da bir Windows ARM64 makinesinin; MSI ve “yükleyici gerçekten kuruyor mu” da öyle. **Kalan:** ikinci bir prova. Linux ARM'in üç formatı burada kanıtlandı, `.ico` iki Windows satırının ortak sebebiydi ve düzeltildi — sıra o iki satırın paketlemeye geçtiğini görmekte |
...
**Kod yazılacak olan kalmadı.** Üçtü; üçü de kapandı ve kayıtları
`CHANGELOG.md`'de. **B-1**'in `exec_ts` varsayımı koddan kaldırıldı: StackVo'nun
kendi başlattığı bir kayıt, isteğin iki yanındaki host saatini taşıyor, ve
tarayıcıda alınmış bir kayıt için kalan hesaplanmış yedek ekranda **hesaplandı**
diye adlandırılıyor (`examples/explain_probe.rs` o yedeğin varsayımını canlı bir
...
* **§2 W / #35** — testlerin Windows'ta **yeşile dönmesi**. İki tur log okundu.
  On dokuzdan geriye biri kalmıştı (`agents::home()`, `%USERPROFILE%`); o
...
* **§2 Y-1** — bu uygulamayı gerçekten bir ekran okuyucuyla kullanmak. Y-2
  ...
  ulaşmıyor. Kalan bir makine değil bir kişi: `npm run a11y:transcript` neyin
  duyurulduğunu yazıyor, kullanmanın nasıl bir şey olduğunu yazamıyor.
* **#22** — Linux ARM yarısı **bu makinede kapandı**: `tools/linux/run.sh
  --bundle` üç formatı da üretti (`deb`/`rpm`/`appimage`, hepsi `aarch64`
  adıyla). İki Windows satırının sebebi de okundu ve Windows'a değil ikona
  aitmiş — `bundle.icon` `.ico`'yu bildirmiyordu, ikisi de aynı yerde
  ölüyordu. Kalan: **ikinci bir prova**, iki Windows satırının paketlemeye
  geçtiğini görmek için.

**Elle bir adım, bir tarih ya da bir süreç:**

* **#35** — Linux yarısı geçiyor (CI #38'de `ubuntu-latest` yeşil) ve Windows
  dalı hem tip kontrolünden geçiyor hem de **koşuyor**. Bu satırın kendi sorusu
  kapandı; geriye kalan *yeşiller mi* sorusu W'nin, ve orada duruyor.
* **#31** — tar'ın elle üretilmesi (`tar -cf … -C <dizin> .`).
* **#2** — "kalan tek eylem bir etiket" yanlıştı ve yanlışlığı üç tur sürdü,
  çünkü kimse log'u açmamıştı. Altı düşüşün üç sebebi vardı; ikisi zaten
  düzeltilmişti, üçüncüsünden geriye bir test kalmıştı ve o da kapandı. Üstelik
