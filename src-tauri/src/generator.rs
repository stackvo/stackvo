...
/// The base image a node project builds on.
///
/// Named rather than inlined into the `FROM` line, because a second reader
/// appeared: [`crate::editor`] has to know whether the userland is glibc or
/// musl before it will attach an editor to the container, and the only honest
/// answer is the image this Dockerfile actually says. Written twice, the two
/// would agree until the day node's tags changed and then disagree silently —
/// and the disagreement would surface as an editor server that unpacks and
/// cannot run.
pub fn node_base_image(version: &str) -> String {
