...
///
/// One: the official registry's content key, generated by ADR 0015's ceremony
...
/// they are the same.
///
/// Adding to this list is a deliberate act with documents to update, and
/// `an_added_key_is_a_deliberate_act` is what makes that true rather than
/// hoped: `docs/durum.md` §6 records the ceremony, and the sentences that used
/// to say the chain was open are now wrong wherever they survive.
...
///
/// It matters because of ADR 0015. The updater key and the content key are
/// deliberately separate, and the thing that makes paying for two signing
...
/// would be a second procedure, and ADR 0015 says in as many words what that
/// produces: the one nobody maintains.
///
...
QtKMXWyYcwdpZAlPF7tE2ENJkRd1ujvKjlj1m9RtHTBnZPa5WKU5uWRs5GoP5M/VqE81QFuMKI5k/SfNQUaOAA==";
    ...
    ///
    /// ADR 0015 keeps the updater key and the content key separate and pays for
    /// that with two signing surfaces; what makes it worth paying is that the
    ...
    /// is the second procedure ADR 0015 says goes unmaintained.
    #[test]
    ...
    fn an_added_key_is_a_deliberate_act() {
        assert_eq!(
            ...
            "the pinned set changed size. A build trusts every key in it, so \
             adding one is a decision for docs/durum.md §6 rather than a diff."
        );
    ...
    ///
    /// ADR 0015's whole point, checked from this side as well as from
